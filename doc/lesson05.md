...
### Внимание: для JDK 16 обновите IDEA на последнюю версию и Tomcat на 9.x
> - Проект обновил до JDK 16. Для запуска Maven или Tomcat переопредели переменную окружения `JAVA_HOME` и переменную `path`, чтобы  `java -version` тоже было 16. Напомню, что IDEA это java процесс. Чтобы новые переменные окружения в ней увиделись, требуется ее перегрузить.
